...
void QgShader::ReadSrc(const char *pFileName)
{
    FILE *f = NULL;
    int readBytes = 0;
    char buf[QG_SHADER_SRC_BUF];
    ...
    while(!feof(f))
    {
        readBytes = fread(buf, 1, QG_SHADER_SRC_BUF, f);
        assert(readBytes < QG_SHADER_SRC_BUF);
        m_src.append(buf, readBytes);
    }
    fclose(f);
} 

QgShader::QgShader(void)
{
    ;
} 

unsigned int QgShader::Compile(GLenum type)
{
   GLuint shaderId = 0;
   GLint compiled = 0;
