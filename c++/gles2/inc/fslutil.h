...
typedef enum fslStatus_e
{
    ...
    /*PP change 0xFFFFFFFF to 0-1 This forces the data type for binary lib compatipility */
    FSL_FSLSTATUS_END             = 0-1
} fslStatus;
...
typedef enum fslInputType_e
{
    ...
    FSL_INPUTTYPE_END      =0-1 /* pp change 0xFFFFFFFF to 0-1 */
} fslInputType;

typedef struct fslInputEventType_s
{
    fslInputType input;
    /* X, keystroke 1  Coordinates range from 0 to FSL_POINTER_MAX; 0,0 lower left corner */
    unsigned int signalA;  
...
typedef enum fslInputDevice_e
{
    ...
    FSL_INPUTDEVICETYPE_END        =0-1 /* pp change 0xFFFFFFFF to 0-1 */
} fslInputDevice;
